...
-- 급여 기준으로 내림차순 정렬을 수행할 수 있도록 한다.ㄴ
...
--   쿼리문을 구성한다.
SELECT FILENO "파일번호"
     , REVERSE(FILENAME) "뒤집자"
     , INSTR(REVERSE(FILENAME),'\',1) "\인덱스찾자"
     , SUBSTR(REVERSE(FILENAME),1,INSTR(REVERSE(FILENAME),'\',1)-1) "뒤집기만하면 끝"
     , REVERSE(SUBSTR(REVERSE(FILENAME),1,INSTR(REVERSE(FILENAME),'\',1)-1)) "끝"
FROM TBL_FILES;
--==>>
/*
1	COD.SELAS\CCC\BBB\AAA\:C	                10	COD.SELAS	            SALES.DOC
2	SLXX.EAMNAP\AAA\:C	                        12	SLXX.EAMNAP	            PANMAE.XXLS
3	TPP.HCRAESER\:D	                            13	TPP.HCRAESER	        RESEARCH.PPT
4	PWH.YDUTS\STNEMUCOD\:C	                    10	PWH.YDUTS	            STUDY.HWP
5	TXT.LQS\PMET\STNEMUCOD\:C	                 8	TXT.LQS	                SQL.TXT
6	GNP.TSET\F\ERAHS\:D	                         9	GNP.TSET	            TEST.PNG
7	GPJ.GNIRPS\OTOHP\ERUTCIP\GNODLIUG\RESU\:C	11	GPJ.GNIRPS	            SPRING.JPG
8	LQS.TTOCS_10_10901202\YDUTSELCARO\:C	    22	LQS.TTOCS_10_10901202	20210901_01_SCOTT.SQL
*/
...
SELECT REVERSE(FILENAME) 
    ...
    , SUBSTR(REVERSE(FILENAME),1,INSTR(REVERSE(FILENAME),'\',1,1)-1)
