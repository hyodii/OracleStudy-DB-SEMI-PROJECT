...
MILLER		    널이네?백기올려
*/
SELECT ENAME, COMM, NVL2(COMM, '널아니네? 두 번째반환','널이네? 세 번째반환') "확인여부"
FROM TBL_EMP;
/*
SMITH		    널이네? 세 번째반환
ALLEN	300	    널아니네? 두 번째반환
WARD	500	    널아니네? 두 번째반환
JONES		    널이네? 세 번째반환
MARTIN	1400	널아니네? 두 번째반환
BLAKE		    널이네? 세 번째반환
CLARK		    널이네? 세 번째반환
SCOTT		    널이네? 세 번째반환
KING		    널이네? 세 번째반환
TURNER	0	    널아니네? 두 번째반환
ADAMS		    널이네? 세 번째반환
JAMES		    널이네? 세 번째반환
FORD		    널이네? 세 번째반환
MILLER		    널이네? 세 번째반환
...
-- ++ NULLIF()함수
-- 첫 번째 매개변수값과 두 번째 매개변수 값이 동일하면 NULL을 그렇지 않으면 매개변수 1을 반환!
SELECT JOB, NULLIF(JOB,'SALESMAN') "NULLIF사용"
FROM TBL_EMP;
--==>>
/*
CLERK	    CLERK
...
--      --------   -------  ------- -----   ----------
SELECT '효진이는', SYSDATE,'에 연봉', 500,'억을 원한다.'
FROM DUAL;
--==>> 효진이는	2021-08-27	에 연봉	500	억을 원한다.
