...
/*
배운 것
1. COUNT() 함수 내부에 DECODE 함수나 CASE문 등을 사용하여 건수를 집계 할 수 있다.(COUNT SUM AVG 도 다 가능한둣?)
2. 집계함수(MAX, MIN)는 WHERE 절에 직접 사용은 할 수 없으며, 서브 쿼리와 같은 방법을 사용해야 한다.
3. WHERE 절안에 조건 비교시 SELECT 문 올 수 있다는 것!
4. GROUP BY 절 안에 컬럼명 여러개 가능하다! (,)로 구분
5. TO_DATE / TO_CHAR / TO_NUMBER 사용하는법!
*/
