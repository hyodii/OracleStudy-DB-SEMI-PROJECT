...
--○ 중도포기 INSERT 프로시저
--> 중도포기 레코드를 입력 시, 
-- 1. "과정 시작일 < 중도포기일 < 과정종료일"이 맞는지
-- 2. 기존에 포기하지 않은 과정이 맞는지 확인하는 프로시저
...
BEGIN
    -- 예외 처리1 : "과정 시작일 < 중도포기일 < 과정종료일"이 아닐 경우
    ...
    EXCEPTION
        ...
        WHEN USER_DEFINE_ERROR2
            THEN RAISE_APPLICATION_ERROR(-20002, '이미 포기한 과정입니다.');
                 ROLLBACK;
        WHEN OTHERS
            THEN ROLLBACK;
END;
...
BEGIN
    IF (INSERTING)
        THEN INSERT INTO PRO_EVENTLOG(MEMO)
            VALUES('교수정보 INSERT 쿼리문이 수행되었습니다.');
    ELSIF (UPDATING)
        THEN INSERT INTO PRO_EVENTLOG(MEMO)
            VALUES('교수정보 UPDATE 쿼리문이 수행되었습니다.');
    END IF;
END;
...
BEGIN
    ...
    EXCEPTION
        ...
        WHEN OTHERS
            THEN ROLLBACK;
END;
--==>> Procedure PRC_COR_DELETE이(가) 컴파일되었습니다.
